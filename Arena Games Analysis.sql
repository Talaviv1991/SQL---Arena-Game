--Arena Games Analysis

USE arena

--Payments Analysis by player and gender:

--01- Generating a report that shows each player a single payment method by priority
...
--02 - Report showing the number of players carrying each type of card, by gender and age group
-------------------------------------------------------
...
--03 - Report displaying the number of sessions for each game and their ranking
...
--04 - Ranking games by the amount of time (minutes) played
...
--05- Most Played Games by Age Group
...
--07-one way -Session Outcome Analysis
...
--7-second way - Session Outcome Analysis
...
--08 Session Outcome Analysis by gender
;WITH balance_cte AS 
	(
	SELECT session_id, action_id,
		   SUM(CASE WHEN action_type = 'loss' THEN amount * -1 ELSE amount END)
...
--09-one way  -Total Profit/Loss per Player
...
--9-second way - Total Profit/Loss per Player
...
--11 - Company Profit/Loss Report by Year and Quarter
...
--12 - Company's Best and Worst 3 Months (by Profit/Loss)
...
-- 12 Alternative Approach to Company's Best and Worst 3 Months 
;WITH balance_cte AS 
	(
	SELECT session_id, action_id,
		   SUM(CASE WHEN action_type = 'loss' THEN amount * -1 ELSE amount END)
